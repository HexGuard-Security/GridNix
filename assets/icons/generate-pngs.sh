...
# Convert the main GridNix icon
if [ -f "${ICON_DIR}/GridNix-icon.svg" ]; then
    mkdir -p "${OUTPUT_DIR}/16" "${OUTPUT_DIR}/32" "${OUTPUT_DIR}/48" "${OUTPUT_DIR}/128" "${OUTPUT_DIR}/256"
    convert_svg_to_png "${ICON_DIR}/GridNix-icon.svg" "${OUTPUT_DIR}"
else
    echo "⚠️ Warning: Main GridNix icon (GridNix-icon.svg) not found"
fi
